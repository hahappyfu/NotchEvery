...
// TestInspect_CreditsExhaustedTriggersSwitchAndCool covers paid-model credits
// running out (e.g. DeepSeek billed per credit): the 403 names the account's
// balance, not the global free-tier queue, so the pool must cool that account
// under a distinct reason until the daily rollover and rotate to the next one.
func TestInspect_CreditsExhaustedTriggersSwitchAndCool(t *testing.T) {
	...
	// Sticky user_1 first, so the cool path has something to clear.
	if _, err := pool.Current(0, nil); err != nil {
		t.Fatalf("Current(0) failed: %v", err)
	}
	if pool.stickyUser != "user_1" {
		t.Fatalf("sticky user = %q, want user_1 before the credits hit", pool.stickyUser)
	}
	...
	cases := []struct {
		...
		{"insufficient credits", `{"error":"insufficient credits for model deepseek"}`},
		{"credit balance", `{"message":"credit balance is zero"}`},
		{"点数不足", `{"message":"点数不足，请充值"}`},
		{"余额不足", `{"message":"余额不足"}`},
		{"points exhausted", `{"message":"points exhausted"}`},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			pool.mu.Lock()
			pool.accounts["user_1"].Cooled = false
			pool.accounts["user_1"].CoolReason = ""
			pool.stickyUser = "user_1"
			pool.mu.Unlock()

			if v := pool.Inspect("user_1", 403, []byte(tc.body)); v != VerdictSwitch {
				t.Fatalf("verdict = %v, want VerdictSwitch", v)
			}
			...
			if !acc.Cooled {
				t.Fatal("credits exhaustion did not cool the account")
			}
			if acc.CoolReason != "credits_exhausted" {
				t.Errorf("cool reason = %q, want credits_exhausted", acc.CoolReason)
			}
			if want := nextLocalMidnight(); !acc.CooledUntil.Equal(want) {
				t.Errorf("cooled until = %v, want next midnight %v", acc.CooledUntil, want)
			}
			if acc.LastFailedAt.IsZero() {
				t.Error("LastFailedAt not stamped on credits exhaustion")
			}
			if pool.stickyUser != "" {
				t.Errorf("sticky user = %q, want cleared", pool.stickyUser)
			}
		})
	}

	// A credits_exhausted account must stay out of rotation for the rest of the
	// day: Current() skips every cooled account regardless of reason.
	c, err := pool.Current(0, nil)
	if err != nil {
		t.Fatalf("Current(0) after cool failed: %v", err)
	}
	if c.UserID != "user_2" {
		t.Errorf("picked %s, want user_2 (user_1 stays cooled until midnight)", c.UserID)
	}
}
