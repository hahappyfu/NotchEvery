...
## 补丁 1（视频对齐 + 右键菜单，2026-09-04 晚追加）

用户两段视频确认的终态（PixPin_23-31-37 为完整参考）：

1. **虚影改舌头形**：宽 = max(刘海宽 × 1.3, 240)、高 60、底角 16、底色 `#2a2c33`（现 +16/+6 微扩作废）
2. **点击过桥菊花**：`openFromGhost()` 后挂 150ms 短菊花（重建 SpinnerView，仅点击触发的过桥反馈，与已删的 hover 预备拍无关）
3. **内容对焦入场**：StaggeredEntry 加 `.blur(4→0)`
4. **收起两段式**：先缩回虚影尺寸 200ms（`closeToGhost`），再清态回刘海
5. **右键菜单**：刘海外壳挂 `.contextMenu`：「打开设置」→ `vm.showSettings()`（需先 `openFromGhost()` 进展开态再切 settings）；「退出 NotchNook」→ `NSApp.terminate(nil)`。虚影态与展开态均可右击。
